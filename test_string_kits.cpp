...
TEST_SUITE("Split Operations") {
    ...
    TEST_CASE("split_by_any empty string with delimiters") {
        std::string test_str = "";
        auto parts = split_by_any(test_str, ",;");
        CHECK(parts.empty());
    }

    TEST_CASE("split_by_any with consecutive delimiters at start") {
        std::string test_str = ",,hello";
        auto parts = split_by_any(test_str, ",");
        CHECK(parts.size() == 3);
        CHECK(parts[0].empty());
    ...
    TEST_CASE("split_by_any with consecutive delimiters at end") {
        std::string test_str = "hello,,";
        auto parts = split_by_any(test_str, ",");
        CHECK(parts.size() == 3);
        CHECK(parts[0] == "hello");
    ...
    TEST_SUITE("Match Operations") {
        ...
        TEST_CASE("Match with initializer_list") {
            CHECK(match_any("target", {"target1", "target", "target2"}) ==
                  true);
        ...
        TEST_CASE("match_any with initializer_list basic") {
            CHECK(match_any("target", {"target1", "target", "target2"}) ==
                  true);
            CHECK(match_any("missing", {"a", "b", "c"}) == false);
        }

        TEST_CASE("match_any with initializer_list single target") {
            CHECK(match_any("test", {"test"}) == true);
            CHECK(match_any("test", {"other"}) == false);
        }

        TEST_CASE("match_any with initializer_list empty list") {
            CHECK(match_any("test", {}) == false);
        }

        TEST_CASE("match_any with initializer_list case sensitive") {
            CHECK(match_any("Hello", {"hello", "HELLO"}) == false);
            CHECK(match_any("Hello", {"Hello", "hello"}) == true);
        }

        TEST_CASE("match_any with initializer_list numeric conversion") {
            CHECK(match_any(42, {"42", "43"}) == true);
            CHECK(match_any(99, {"100", "101"}) == false);
        }

        TEST_CASE("match_any with initializer_list empty string") {
            CHECK(match_any("", {""}) == true);
            CHECK(match_any("", {"test"}) == false);
        }
    }
    ...
    TEST_SUITE("Contains Operations") {
        ...
        TEST_CASE("contains_any returns false if none exist") {
            std::string test_str = "Hello World Test";
            std::array<std::string_view, 3> search_strings = {"XYZ", "ABC",
                                                              "DEF"};
        ...
        TEST_CASE("contains_any with empty search strings") {
            std::string test_str = "Hello World";
            CHECK(contains_any(test_str, std::vector<std::string_view>{}) ==
                  false);
        }

        TEST_CASE("contains_all with empty search strings") {
            std::string test_str = "Hello World";
            CHECK(contains_all(test_str, std::vector<std::string_view>{}) ==
                  false);
        }

        TEST_CASE("contains_any with initializer_list") {
            std::string test_str = "Hello World Test";
            CHECK(contains_any(test_str, {"Hello", "World", "Test"}) == true);
        }

        TEST_CASE(
            "contains_any with initializer_list returns false if none exist") {
            std::string test_str = "Hello World Test";
            CHECK(contains_any(test_str, {"XYZ", "ABC", "DEF"}) == false);
        }

        TEST_CASE("contains_any with initializer_list single match") {
            std::string test_str = "Hello World";
            CHECK(contains_any(test_str, {"World"}) == true);
            CHECK(contains_any(test_str, {"XYZ"}) == false);
        }

        TEST_CASE("contains_any with initializer_list empty list") {
            std::string test_str = "Hello World";
            CHECK(contains_any(test_str, {}) == false);
        }

        TEST_CASE("contains_any with initializer_list empty source") {
            CHECK(contains_any("", {"test"}) == false);
        }

        TEST_CASE("contains_all with initializer_list") {
            std::string test_str = "Hello World Test";
            CHECK(contains_all(test_str, {"Hello", "World"}) == true);
        }

        TEST_CASE(
            "contains_all with initializer_list returns false if missing") {
            std::string test_str = "Hello World Test";
            CHECK(contains_all(test_str, {"Hello", "XYZ"}) == false);
        }

        TEST_CASE("contains_all with initializer_list single element") {
            std::string test_str = "Hello World";
            CHECK(contains_all(test_str, {"Hello"}) == true);
            CHECK(contains_all(test_str, {"XYZ"}) == false);
        }

        TEST_CASE("contains_all with initializer_list empty list") {
            std::string test_str = "Hello World";
            CHECK(contains_all(test_str, {}) == false);
        }

        TEST_CASE("contains_all with initializer_list empty source") {
            CHECK(contains_all("", {"test"}) == false);
        }
    }
     ...
     * Tests for string removal operations including single and multiple
     * patterns.
     */
    TEST_SUITE("Remove Operations") {
        ...
        TEST_CASE("remove_all non-overlapping behavior example") {
        ...
        TEST_CASE("remove_all with initializer_list") {
            std::string test_str = "hello world hello world";
            remove_all(test_str, {"hello", "world"});
            CHECK(test_str == "   ");
        }

        TEST_CASE("remove_all with initializer_list single pattern") {
            std::string test_str = "hello world hello";
            remove_all(test_str, {"hello"});
            CHECK(test_str == " world ");
        }

        TEST_CASE("remove_all with initializer_list empty list") {
            std::string test_str = "hello world";
            remove_all(test_str, {});
            CHECK(test_str == "hello world");
        }

        TEST_CASE("remove_all with initializer_list no matches") {
            std::string test_str = "hello world";
            remove_all(test_str, {"xyz", "abc"});
            CHECK(test_str == "hello world");
        }

        TEST_CASE("remove_all with initializer_list empty source") {
            std::string test_str = "";
            remove_all(test_str, {"test"});
            CHECK(test_str.empty());
        }

        TEST_CASE("remove_all with initializer_list overlapping patterns") {
            std::string test_str = "aaa";
            remove_all(test_str, {"a", "aa"});
            CHECK(test_str.empty());
        }

        TEST_CASE("remove_all non-overlapping behavior example") {
            std::string test_str = "aabbaa";
            remove_all(test_str, "ab");
            CHECK(test_str == "abaa");
        }

        TEST_CASE("remove_all non-overlapping behavior with single pattern") {
            std::string test_str = "aabbaa";
            remove_all(test_str, "ab");
            CHECK(test_str == "abaa");
        }

        TEST_CASE("get_all_removed with initializer_list") {
            std::string test_str = "test123test456test";
            auto removed = get_all_removed(test_str, {"test", "123"});
            CHECK(removed == "456");
            CHECK(test_str == "test123test456test");
        }
    }
